...
function App() {
  // === Custom Hooks ===
  const { wordList, isLoadingData, dataError, currentDataVersion } =
    useWordData();
  const {
    currentPair,
    languageDirection,
  ...
  useEffect(() => {
    console.log(
      `App.jsx Effect: wordList (len ${
        wordList.length
      }), isLoadingData (${isLoadingData}), currentPair (${!!currentPair}), dataError (${!!dataError}), gameError (${!!gameError})`
    );
    if (
      !isLoadingData &&
      wordList.length > 0 &&
      ...
      console.log(
        "App.jsx: wordList ready, no current pair, no errors. Selecting initial pair via hook."
      );
      selectNewPairCard();
    } else if (
      !isLoadingData &&
  ...
  const handleToggleHardWordsView = () => {
    setShowHardWordsView((prev) => {
      if (!prev) {
        // If we are about to show hard words view
        ...
        <>
          {!isLoadingData && !dataError && !gameError && currentPair && (
            <div className="flashcard-area">
              ...
              />
              {showFeedback &&
                feedbackSignal === "incorrect" && ( // showFeedback & feedbackSignal from useFlashcardGame
                  <div className="feedback-area">
                    <p>
                      Incorrect. The correct answer is: "{lastCorrectAnswer}"
                    </p>{" "}
                    {/* lastCorrectAnswer from useFlashcardGame */}
                    <button
          ...
          {!isLoadingData &&
            ...
            !gameError &&
            !currentPair &&
            wordList.length > 0 && <p>No card available. Try "New Card".</p>}
          {!isLoadingData &&
            !dataError &&
            !gameError &&
            !currentPair &&
