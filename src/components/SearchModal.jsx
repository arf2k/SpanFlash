...
const SearchModal = ({ isOpen, onClose, wordList, onSelectResult }) => {
    ...
    // Updated useEffect to filter wordList including synonyms
    useEffect(() => {
        if (!searchTerm.trim()) {
            setSearchResults([]);
            return;
        ...
        if (wordList && wordList.length > 0) {
            ...
            const filtered = wordList.filter(pair => {
                ...
                if (pair.spanish && pair.spanish.toLowerCase().includes(lowerSearchTerm)) {
                    match = true;
                }
                // Check primary English field
                if (!match && pair.english && pair.english.toLowerCase().includes(lowerSearchTerm)) {
                    match = true;
                }
                // Check Spanish synonyms (if they exist and are an array)
                if (!match && pair.synonyms_spanish && Array.isArray(pair.synonyms_spanish)) {
                ...
                if (!match && pair.synonyms_english && Array.isArray(pair.synonyms_english)) {
                            ...
                            <ul className="search-results-list">
                                {searchResults.map((pair, index) => (
                                    <li
                                        ...
                                        tabIndex={0}
                                        role="button"
                                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleResultClick(pair);}}
                                    >
