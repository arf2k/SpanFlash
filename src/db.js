...
db.version(3)
  ...
  .upgrade(async (tx) => {
    console.log(
      "Upgrading database schema to version 3 for Leitner System fields..."
    );
    const now = Date.now();
    await tx
      .table("allWords")
...
db.version(5)
  ...
  .upgrade(async (tx) => {
    ...
    console.log(
      `Database upgrade to version 5 successful. All ${count} existing words moved to Leitner Box 0.`
    );
  });
db.version(6).stores({
  appState: "id",
...
db.version(7)
  .stores({
    ...
    hardWords: "[spanish+english]",
    allWords:
      "++id, spanish, english, category, dueDate, leitnerBox, exposureLevel",
    dailyStats: "date",
  })
  .upgrade(async (tx) => {
    console.log(
      "Upgrading database to version 7: Adding exposure tracking system..."
    );
    ...
    await tx
      ...
      .modify((word) => {
        ...
        word.timesCorrect = Math.max(
          Math.floor((word.leitnerBox || 0) * 0.7),
          0
        );
    ...
    const count = await tx.table("allWords").count();
    console.log(
      `Database upgrade to version 7 successful. Migrated ${count} words to exposure system.`
    );
  });
db.version(8)
  .stores({
...
db.version(9)
  .stores({
    ...
    allWords:
      "++id, spanish, english, category, dueDate, leitnerBox, exposureLevel",
    dailyStats: "date",
    incompleteWords: "++id, spanish, extractedAt, status",
  })
  .upgrade(async (tx) => {
    console.log("Adding incomplete words table for vocabulary extraction...");

    console.log("✅ Incomplete words table added successfully");
  });
